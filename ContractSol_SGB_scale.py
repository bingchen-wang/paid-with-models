...
def ContractSol_SGB(N, p, c, k= None, a_funcs = None, v_funcs = None, exact_Hessian = True, maxiter = 1000, verbose = False, bounds = False):
    ...
    def make_SGB(a_opt, k):
        def a(x):
            ...
            if np.isnan(result).any() or np.isinf(result).any():
                print("NaN or inf encountered in a:", result)
            return result
        ...
        def a_der(x):
            ...
            if np.isnan(result).any() or np.isinf(result).any():
                print("NaN or inf encountered in a_der:", result)
        ...
        def a_hess(x):
            ...
            if np.isnan(result).any() or np.isinf(result).any():
                print("NaN or inf encountered in a_hess:", result)
            return result
    ...
    #  N.b. the conversion of the maximisation problem to a minimisation problem
    def obj_f(x):
        result = -sum(prob_outcomes * a(all_outcomes @ x[:int(len(x)/2)]))
        if np.isnan(result) or np.isinf(result):
            print("NaN or inf encountered in obj_f:", result)
        return result
    ...
    def Jacobian_f(x):
        ...
        if np.isnan(J).any() or np.isinf(J).any():
            print("NaN or inf encountered in Jacobian_f:", J)
        return -J
    ...
    def expected_value(x, i = None):
        ...
        result = sum(probs * v(a(outcomes @ x[:int(len(x)/2)])))
        if np.isnan(result) or np.isinf(result):
            print("NaN or inf encountered in expected_value:", result)
        return result
    ...
    def linearConstraint(K):
        ...
        if np.isnan(A).any() or np.isinf(A).any():
            print("NaN or inf encountered in linear constraint matrix A:", A)
    ...
    def NC_f(z):
        ...
        if np.isnan(output).any() or np.isinf(output).any():
            print("NaN or inf encountered in NC_f:", output)
        return output
    ...
    def Jacobian_NC(z):
        ...
        J_NC = np.vstack([sec_1, sec_2, sec_3])
        if np.isnan(J_NC).any() or np.isinf(J_NC).any():
            print("NaN or inf encountered in Jacobian_NC:", J_NC)
        return J_NC
    ...
    # <Step 7> ----- Find a good starting point (compute the pooling contract) -----
    def pool_f(m):
        result = v(a(N*m)) - c[0] * m - f[0]
        if np.isnan(result) or np.isinf(result):
            print("NaN or inf encountered in pool_f:", result)
        return result
